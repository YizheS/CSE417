...
public class OptimalLineup {
    ...
    for (int i = 0; i < defenders.size(); i++) {
      for (int j = i + 1; j < defenders.size(); j++) {
        for (int k = j + 1; k < defenders.size(); k++) {
          for (int u = k + 1; u < defenders.size(); u++) {
            for (int v = u + 1; v < defenders.size(); v++) {
              Lineup lineup = new Lineup(budget, 3);
              if (lineup.canAdd(defenders.get(i))) {
                lineup.add(defenders.get(i));
              } else {
                continue;
              }
              if (lineup.canAdd(defenders.get(j))) {
                lineup.add(defenders.get(j));
              } else {
                continue;
              }
              if (lineup.canAdd(defenders.get(k))) {
                lineup.add(defenders.get(k));
              } else {
                continue;
              }
              if (lineup.canAdd(defenders.get(u))) {
                lineup.add(defenders.get(u));
              } else {
                continue;
              }
              if (lineup.canAdd(defenders.get(v))) {
                lineup.add(defenders.get(v));
              } else {
                continue;
              }
              ...
              if (points > bestPointsDEF[price]) {
                ...
                bestPlayersDEF[price] = lineup.getPlayers().toArray(new Player[lineup.size()]);
              }
              countDEF++;
            }
          }
  ...
  private int tryAttackingLineups(Lineup lineup, int start) {
    if (lineup.size() == 8) {
      if (lineup.getExpectedPoints() + bestPointsDefense[lineup.getRemainingBudget()] > bestPoints) {
        bestPoints = lineup.getExpectedPoints() + bestPointsDefense[lineup.getRemainingBudget()];

        // change bestLineup
        Lineup newBestLineup = lineup.clone();
        Player[] bestPlayersDefAtRemainingBudget = bestPlayersDefense[lineup.getRemainingBudget()];
        for (int i = 0; i < bestPlayersDefAtRemainingBudget.length; i++) {
          newBestLineup.add(bestPlayersDefAtRemainingBudget[i]);
        }
        bestLineup = newBestLineup;
      }
      return 1;
    }

    LineupBound upperLineupBound = new LineupBound(lineup, 0, 0, 0,
            5 - lineup.sizeByPosition(Position.MID), 3 - lineup.sizeByPosition(Position.FWD));
    float upperBound = upperLineupBound.maxPoints(attackers.subList(start, attackers.size()).iterator());
    upperBound += bestPointsDefense[lineup.getRemainingBudget()];

    if (upperBound - bestPoints <= 1e-5) {
      return 1;
    }

    int count = 1;
    for (int i = start; i < attackers.size(); i++) {
      if (lineup.canAdd(attackers.get(i))) {
        lineup.add(attackers.get(i));
        count += tryAttackingLineups(lineup, i + 1);
        lineup.remove(attackers.get(i));
      }
    }
    return count;
  }
